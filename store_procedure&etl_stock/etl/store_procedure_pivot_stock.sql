CREATE OR REPLACE PROCEDURE pivot_tb_stock_no(tb_name text) as $$
DECLARE
    cols_name text;
    query text := '';
    final_query text := '';
BEGIN
    ...
    final_query := format(
        'CREATE TABLE staging.%I_stock_sheet1_stock AS
        SELECT *,
          CASE 
...
CALL pivot_tb_stock('stock_sheet1');